# app.py - Enhanced Recommendation System Interface
import os
os.environ['TF_CPP_MIN_LOG_LEVEL'] = '3'  # Suppress TensorFlow info messages
os.environ['TF_ENABLE_ONEDNN_OPTS'] = '0'  # Disable oneDNN message specifically
...
class RecommendationSystem:
    ...
    def load_movie_titles(self):
        ...
        try:
            # Try different possible locations for the movie data
            possible_paths = [
                os.path.join(self.data_dir, 'movies_clean.csv'),
            ...
            for path in possible_paths:
                if os.path.exists(path):
                    ...
                    print(f"Loaded {len(self.movie_titles)} movie titles from {path}")
                    return
            
            print("Warning: Could not find movies_clean.csv file")
            self.movie_titles = {}
    ...
    def get_recommendations(self, user_id, top_n=10):
        ...
        user_id_map = self.mappings.get('user_id_to_idx', {})
        movie_id_map = self.mappings.get('movie_id_to_idx', {})
        ...
        movie_indices = list(movie_id_map.values())
    ...
    def display_recommendations(self, recommendations):
        ...
        for i, (movie_id, rating) in enumerate(recommendations, 1):
            # Get movie title or default to ID if not found
